...
def generate_path(N, M):
    ...
    # your code here
    directions = ['up','down','left','right'] #把四個方向先寫出來
    direction = random.choice(direction) #從四個方向中隨機選
    #不會寫了......
    return maze
...
def add_obstacles(maze, num_obstacles):
    ...
    # your code here
    new_value=x
    while num_obstacles <= (N-1)*(M-1):
        row_position=random.randint(0,N)
        col_position=random.randint(0,M) #隨機取出位置
        new_value=maze[row_position,col_position] #再把位置放到maze裡面
